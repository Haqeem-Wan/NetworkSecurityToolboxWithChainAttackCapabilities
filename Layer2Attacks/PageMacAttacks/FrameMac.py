...
class FrameMac:
    def __init__ (self, frame):
        ...
        self.selectGenerateMacButton = Button(self.attackFrame, height=1, width=31, font="bahnschrift 15", text="Generate MAC Address", fg="#ffffff", bg="#252525", 
                                  command=lambda : self.switchMacSpoofMode("N/A", "Generate"))
        self.selectGenerateMacButton.place(x = 85, y = 18)
        
        self.selectEnterMacButton = Button(self.attackFrame, height=1, width=31, font="bahnschrift 15", text="Enter MAC Address", fg="#ffffff", bg="#252525", 
                                  command=lambda : self.switchMacSpoofMode(self.targetDefaultGateEntry.get(), "Enter"))
        self.selectEnterMacButton.place(x = 530, y = 18)
    ...
    def switchMacSpoofMode(self, macAddress, mode) :

        if (mode == "Enter") :
            self.selectGenerateMacButton.config(relief="raised")
            self.selectGenerateMacButton.config(state = "normal")
            self.selectEnterMacButton.config(relief="sunken")
            self.selectEnterMacButton.config(state = "disabled")
        else :
            self.selectGenerateMacButton.config(relief="sunken")
            self.selectGenerateMacButton.config(state = "disabled")
            self.selectEnterMacButton.config(relief="raised")
            self.selectEnterMacButton.config(state = "normal")

        self.macSpoofMode = macAddress
        print(mode)